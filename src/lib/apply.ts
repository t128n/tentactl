...
async function deleteUnlistedLabels(
	octokit: Octokit,
	org: string,
	repo: string,
	configuredLabels: LabelConfig[],
): Promise<void> {
...
async function applyCollaborators(octokit: Octokit, config: TentactlConfig): Promise<void> {
	...
	for (const collaborator of items) {
		try {
			...
			consola.success(
				`Collaborator "${collaborator.username}" granted ${collaborator.permission} on ${org}/${repo}`,
			);
		} catch (err: unknown) {
			consola.error(
				`Failed to set permissions for collaborator "${collaborator.username}": ${formatError(err)}`,
			);
		}
	}
...
async function removeUnlistedCollaborators(
	...
	const configuredUsernames = new Set(
		configuredCollaborators.map((collaborator) => collaborator.username),
	);
		...
		for (const collaborator of existing) {
			if (!configuredUsernames.has(collaborator.login)) {
				try {
					...
					consola.error(
						`Failed to remove collaborator "${collaborator.login}": ${formatError(err)}`,
					);
				}
			}
		}
...
async function removeUnlistedTeams(
	octokit: Octokit,
	org: string,
	repo: string,
	configuredTeams: TeamConfig[],
): Promise<void> {
	...
	for (const env of environments) {
		try {
			await octokit.request("PUT /repos/{owner}/{repo}/environments/{environment_name}", {
				...
				...(env.prevent_self_review !== undefined
...
function resolveStrict(
	...
	sectionStrict: boolean | undefined,
): boolean {
