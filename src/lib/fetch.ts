    ...
    try {
        ...
        const items: RulesetConfig[] = nodes.map((node) => {
            ...
            const rules = node.rules.nodes
                ...
                .map((r) => ({
                    ...
                    ...(r.parameters && Object.keys(r.parameters).length > 0
                        ? { parameters: r.parameters as RulesetConfig["rules"] extends Array<infer R> ? (R extends { parameters?: infer P } ? P : never) : never }
                        : {}),
                }));
            ...
            const bypassActors: RulesetBypassActor[] = node.bypassActors.nodes.map((b) => {
                ...
                else if (b.repositoryRoleDatabaseId !== null) actor.repositoryRoleDatabaseId = b.repositoryRoleDatabaseId;
...
async function fetchEnvironments(
    ...
    try {
        ...
        config.environments = environments.map((env) => {
            ...
            if (typeof e.prevent_self_review === "boolean") item.prevent_self_review = e.prevent_self_review;
