...
describe("applyConfig", () => {
    it("applies configured repository state across supported sections", async () => {
        ...
        expect(replaceAllTopics).toHaveBeenCalledWith({ owner: "acme", repo: "platform", names: ["cli"] });
        expect(updateBranchProtection).toHaveBeenCalled();
        expect(createLabel).toHaveBeenCalledWith({ owner: "acme", repo: "platform", name: "bug", color: "ff0000" });
        expect(addCollaborator).toHaveBeenCalledWith({ owner: "acme", repo: "platform", username: "octocat", permission: "push" });
        ...
        expect(request).toHaveBeenCalledWith("PUT /repos/{owner}/{repo}/environments/{environment_name}", expect.objectContaining({
            owner: "acme",
            repo: "platform",
    ...
    it("removes unmanaged collaborators, teams, and labels in strict mode", async () => {
        ...
        expect(removeCollaborator).toHaveBeenCalledWith({ owner: "acme", repo: "platform", username: "hubot" });
        expect(removeRepoInOrg).toHaveBeenCalledWith({ org: "acme", owner: "acme", repo: "platform", team_slug: "ops" });
    });
});
