...
export function getOctokit(auth: AuthConfig): Octokit {
    return new Octokit({
        auth: auth.token,
        baseUrl: auth.endpoint === "github.com"
            ? "https://api.github.com"
            : `https://${auth.endpoint}/api/v3`,
    });
}
...
export async function validateToken(
    ...
    try {
        const octokit = new Octokit({
            auth: token,
            baseUrl: endpoint === "github.com"
                ? "https://api.github.com"
                : `https://${endpoint}/api/v3`,
        });

        const response = await octokit.rest.users.getAuthenticated();
        const expiresAt = response.headers["github-authentication-token-expiration"] as string | undefined;
        ...
        if (expiresAt) {
            ...
            expiryMsg = daysLeft > 0 ? `expires in ${daysLeft} day${daysLeft === 1 ? "" : "s"}` : "expired";
        }
...
Note: The token will be saved to .env.local in your project root.`);
        ...
        const shouldOpen = await confirm({ message: "Open token page in browser?", initialValue: true });
        if (shouldOpen) open(url);
    }

    const token = await password({
        message: "Enter your Personal Access Token:",
        validate: (v) => { if (!v) return "Token is required"; },
    });
